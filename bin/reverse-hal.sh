...
if [ ! -d "$1" -o ! -f "$1/etc/vintf/manifest.xml" ]; then
	echo "Usage: $0 <vendor-folder>"
	exit 1
fi

rm -f broken
for HAL in $(xmlstarlet sel -t -m '//hal/name' -v . -n "$1/etc/vintf/manifest.xml" | grep -vE '^android\.hardware\.'); do
	interface="$(
		xmlstarlet sel \
			-t -m '//hal[./name/text()="'"$HAL"'"]' \
			-v './interface/name' "$1/etc/vintf/manifest.xml"
	)"
	for version in $(xmlstarlet sel \
		-t -m '//hal[./name/text()="'"$HAL"'"]' \
		-v './version' -n "$1/etc/vintf/manifest.xml"); do
